...
def get_company_css() -> str:
    ...
    """
        ...
        log_structured_error(e, page="company_profile_utils", component="get_company_css", operation="BUILD_CSS")
        return ""
...
def object_to_dict(obj):
    ...
    try:
        ...
        log_structured_error(e, page="company_profile_utils", component="object_to_dict", operation="CONVERT_OBJECT_TO_DICT")
        return {}
...
def get_profile_rows_for_excel(company) -> list:
    ...
    data_items = [
        (LABEL_MAP.get(k, k), v)
        for k, v in company_to_label_value(company).items()
        if v not in (None, "", "N/A") and k not in HIDDEN_FIELDS
    ]
    result = []
    for k, v in data_items:
